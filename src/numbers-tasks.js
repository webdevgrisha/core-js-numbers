...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  ...
  const distanceXY = Math.sqrt(Xdistance ** 2 + Ydistance ** 2);
...
function roundToPowerOfTen(num, pow) {
  ...
  const increaseNum = Number(num % tenInPow >= (5 * tenInPow) / 10);
  const roundNum = (Math.floor(num / tenInPow) + increaseNum) * tenInPow;
...
function getFibonacciNumber(index) {
  ...
  for (let i = 1; i < index; i += 1) {
    const prevNum = fibNum[i - 1];
    const current = fibNum[i];
    const nextNum = prevNum + current;
...
function getSumOfDigits(num) {
  // first way using string transform
  // return String(num)
  //   .split('')
  //   .reduce((sum, digit) => sum + +digit, 0);

  // second way using only math
  let reminder = 0;
  let number = num;
  let sum = 0;
